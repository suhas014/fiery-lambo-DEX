...
* liquidity and also the calculated amount of `ether` and `LP` tokens
*/
...
/**
* getTokensAfterRemove: Calculates the amount of `Eth` and `FLP` tokens
...
export const getTokensAfterRemove = async (
  ...
  try {
    ...
    // Similarly we also maintain a ratio for the `FL` tokens, so here in our case
    // Ratio is -> (amount of FL tokens sent back to the user / FL Token reserve) = (LP tokens withdrawn) / (total supply of LP tokens)
    // Then (amount of FL tokens sent back to the user) = (FL token reserve * LP tokens withdrawn) / (total supply of LP tokens)
